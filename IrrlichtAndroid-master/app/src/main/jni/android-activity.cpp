...
void Java_com_ellismarkov_irrlicht_IrrlichtTest_nativeInitGL(JNIEnv *env) {
    importGLInit();
    device = createDevice(video::EDT_OGLES1, dimension2d<u32>(gWindowWidth, gWindowHeight), 16,
                          false, false, false, 0);

    device->setWindowCaption(L"我是窗口");
    driver = device->getVideoDriver();
//    IGUIEnvironment *guienv = device->getGUIEnvironment();
//    if (guienv != NULL) {
//        guienv->addStaticText(L"Hello,irrlicht!", rect<int>(10, 10, 500, 100), true);
//    }
    __android_log_print(ANDROID_LOG_INFO, "Irrlicht", "createDevice r=%d w=%d h=%d", device,
...
void Java_com_ellismarkov_irrlicht_IrrlichtTest_nativeResize(JNIEnv *env, jobject thiz, jint w,
                                                             jint h) {
...
void Java_com_ellismarkov_irrlicht_IrrlichtTest_nativeSendEvent(JNIEnv *env, jobject defaultObj,
                                                                jobject event) {
    ...
    if (action == 0) {
        ...
        __android_log_print(ANDROID_LOG_INFO, "Irrlicht", "left click received %d %f %f", action, x,
                            y);
    } else if (action == 1) {
        ...
        irrevent.MouseInput.ButtonStates ^= irr::EMBSM_LEFT;
        device->postEventFromUser(irrevent);
        __android_log_print(ANDROID_LOG_INFO, "Irrlicht", "left click release %d %f %f", action, x,
                            y);
    } else {
...
void Java_com_ellismarkov_irrlicht_IrrlichtTest_nativeGetStatus(JNIEnv *env, jobject defaultObj,
                                                                jobject status) {
    if (gAppAlive == 0) {
...
void Java_com_ellismarkov_irrlicht_IrrlichtTest_nativeEnvJ2C(JNIEnv *env, jobject defaultObj,
                                                             jstring jsdcardPath) {
    char ligne[1024 + 1];
